...
class Network():
    ...
    def train(self, train_samples, train_labels, *, data_iterator, iteration_steps):
        ...
        with tf.Session(graph=tf.get_default_graph()) as session:
            ...
            # 使用test_data_iterator遍历整个训练集
            def test_data_iterator_for_train(samples, labels, chunkSize):
            ...
            for i, samples, labels in test_data_iterator_for_train(train_samples, train_labels, chunkSize=self.train_batch_size):
                ...
                print('Train Batch Accuracy: %.1f%%' % accuracy)
            
            if train_accuracies:
                print('\nAverage Training Set Accuracy: %.1f%%' % np.average(train_accuracies))
                print('Training Set Accuracy Standard Deviation: %.1f%%' % np.std(train_accuracies))
            else:
                print('\nNo training data processed for accuracy calculation')
    ...
    def test(self, test_samples, test_labels, *, data_iterator):
        ...
        with tf.Session(graph=tf.get_default_graph()) as session:
            ...
            # 处理空confusionMatrices的情况
            if confusionMatrices:
                self.print_confusion_matrix(np.add.reduce(confusionMatrices))
            else:
                print('Confusion    Matrix: No data available')
            ###
    ...
    def accuracy(self, predictions, labels, need_confusion_matrix=False):
        ...
        if need_confusion_matrix:
            ...
            # 使用固定的类别数量生成混淆矩阵
            cm = confusion_matrix(_labels, _predictions, labels=np.arange(num_classes))
    ...
    def visualize_filter_map(self, tensor, *, how_many, display_size, name):
        ...
        # 获取实际形状
        shape = tf.shape(filter_map)
        # 计算正确的display_size
        actual_height = shape[1]
        actual_width = shape[2]
        # 重塑为正确的形状
        filter_map = tf.reshape(filter_map, (how_many, actual_height, actual_width, 1))
        # 添加到摘要
    ...
    def print_confusion_matrix(self, confusionMatrix):
        ...
        try:
            # 检查confusionMatrix是否为空或不是数组
            if confusionMatrix is None or isinstance(confusionMatrix, (int, float)):
            ...
            if total_samples > 0:
                for i, column in enumerate(np.transpose(confusionMatrix, (1, 0))):
                    ...
                    if column_sum > 0:
                        a += (column[i] / column_sum) * (column_sum / total_samples)
            ...
            print('Error printing confusion matrix:', str(e))
